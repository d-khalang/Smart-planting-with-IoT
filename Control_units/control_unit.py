...
class Controler():
    ...
    def subscribe_main_topic(self, levelsList):
        newLevels = list(set(levelsList) - set(self.levelsList))
        removedLevels = list(set(self.levelsList) - set(levelsList))

        # unSubscribe from the old topics 
        for removedLevel in removedLevels:
            self.client.unsubscribe(self.topic + str(removedLevel) + '/#')
            print(F"Subscription removed from {self.topic + str(removedLevel) + '/#'}")
        
        # Subscribe to the new topics
        for newLevel in newLevels:
            self.client.mySubscribe(self.topic + str(newLevel) + '/#')
            # print(F"Subscribed to {self.topic + str(newLevel) + '/#'}")

        # Updating the list of levels
